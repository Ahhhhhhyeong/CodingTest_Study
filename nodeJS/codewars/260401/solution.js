...
function phone(strng, num) {
    const line = strng.split('\n');
    const finds = line.filter(s => s.includes(num));
    if (finds.length === 0) return `Error => Not found: ${num}`;
    if (finds.length > 1) return `Error => Too many people: ${num}`;
    let find = finds[0];
    const name = find.match(/<([^>]+)>/)[1];
    find = find.replace(num, '');
    find = find.replace(`<${name}>`, '');
    find = find.replace(/_/g, ' ');
    find = find.replace(/[^a-zA-Z0-9\.\- ]/g, '');
    find = find.replace(/\s+/g, ' ').trim();
    return `Phone => ${num}, Name => ${name}, Address => ${find}`;        
}
